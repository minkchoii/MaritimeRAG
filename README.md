...
- `full_corpus_v1`: IMO 회의 자료와 선급 Rule/Guidance 본문 통합 검색
- `kr_tables_v1`: KR 표 질의용 구조화 표 전용 인덱스
- `pilot_100`: 100개 문서 파일럿 인덱스

기존 `full_corpus`와 `kr_tables`는 정책 적용 전 v0 기준선으로 로컬에 보존할 수 있지만 UI 기본값은 v1입니다.
...
이미 `full_corpus_v1`과 `kr_tables_v1` 인덱스가 준비되어 있다면 다음 명령으로 UI를 실행합니다.
...
- 일반 검색 collection: `full_corpus_v1`
- 표 검색 collection: `kr_tables_v1`
...
```powershell
python scripts/35_build_bm25_index.py --unified full_corpus_v1 --rebuild
```
...
표 인덱스는 `kr_tables_v1` collection으로 분리하여 운영합니다. `table_schema`, `table_summary`, `table_markdown`, `table_row`만 포함하고 일반 본문은 제외합니다.
...
```powershell
python scripts/rag_query.py --unified full_corpus_v1 -i --full-text --top-k 5
```
